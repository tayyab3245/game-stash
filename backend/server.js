...
// // Helpers
// const runDbQuery = (sql, params = []) => new Promise((resolve, reject) => {
...
//   const { title, region, fileName, fileSizeMB, isPatched } = req.body;
//   const imageUrl = req.file ? `/covers/${req.file.filename}` : null;

//   try {
//     const result = await runDbQuery(`
//       INSERT INTO games (title, region, fileName, fileSizeMB, isPatched, imageUrl)
//       VALUES (?, ?, ?, ?, ?, ?)`,
//       [title, region, fileName, fileSizeMB, isPatched ? 1 : 0, imageUrl]
//     );
...
//   const allowedFields = ['title', 'region', 'fileName', 'fileSizeMB', 'isPatched'];
//   const fields = allowedFields.filter(key => key in req.body);
//   if (!fields.length) return res.status(400).json({ error: 'No valid fields provided.' });

//   const sql = `UPDATE games SET ${fields.map(f => `${f} = ?`).join(', ')} WHERE id = ?`;
//   const params = [...fields.map(f => req.body[f]), req.params.id];

//   try {
//     await runDbQuery(sql, params);
//     console.log(`[PUT] /api/${req.params.id} — Updated fields: ${fields.join(', ')}`);
...
//     if (game && game.imageUrl) fs.unlinkSync(path.join(__dirname, game.imageUrl));
...
//         INSERT INTO games (title, region, fileName, fileSizeMB, isPatched, imageUrl)
//         VALUES (?, ?, ?, ?, ?, ?)`,
//         [g.title, g.region, g.fileName, g.fileSizeMB, g.isPatched, g.imageUrl]
//       );
//     }
...
app.put('/api/:id', async (req, res) => {
  ...
  const allowedFields = ['title', 'platform', 'romPath', 'emuPath', 'region', 'fileName', 'fileSizeMB', 'isPatched', 'imageUrl'];
  const fieldsToUpdate = allowedFields.filter(key => key in req.body);
  if (!fieldsToUpdate.length) return res.status(400).json({ error: 'No valid fields provided.' });
  ...
  try {
    await runDbQuery(sql, params);
    console.log(`[PUT] /api/${req.params.id} — Updated fields: ${fieldsToUpdate.join(', ')}`);
    res.json({ status: 'UPDATE ITEM SUCCESSFUL' });
  } catch (err) {
    console.error(`[PUT] /api/${req.params.id} — Error:`, err.message);
    res.status(500).json({ error: err.message });
