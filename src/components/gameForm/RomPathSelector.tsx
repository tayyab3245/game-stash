...
const RomPathSelector: React.FC<RomPathSelectorProps> = ({ romPath, onRomPathChange, isSegmented = false }) => {
  ...
  const segmentedBtn: React.CSSProperties = {
    ...
    borderTopLeftRadius: '48px',  // Match modal roundness
    borderBottomLeftRadius: '48px',  // Match modal roundness
    outline: 'none',
    cursor: 'pointer',
    margin: 0,
      ...
      <>
        ...
        >
          ...
          <div style={{ fontSize: 20, opacity: 0.8 }}>
            {romName ? romName.substring(0, 15) + (romName.length > 15 ? '...' : '') : "Select"}
          </div>
        </button>
        <input
          type="file"
